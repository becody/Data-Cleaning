...
# Merge public and private schools
  # Read in datasets 
private <- read.csv("csv-string ",
                      colClasses = c("character"),
                      na.strings = c("","NA"))
public <- read.csv("csv-string",
                            colClasses = c("character"),
                            na.strings = c("","NA"))
# For this file, I removed the path names and other identifying demographic or company-specific information.
...
# Repeat this type of work with students by race/ethnicity variable
...
# Remove extra columns and duplicate NCES IDs 
df2 <- work2 %>%
...
# Re-organize the columns in the dataset to reflect the current order of the schools database
names(df2)
...
# Repeat the encoding with mailing state id
...
write.csv(char3, "clean_data.csv", 
          row.names = FALSE)
